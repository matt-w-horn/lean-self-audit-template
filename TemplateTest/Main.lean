...
public import TemplateTest.Gate

/-!
# The test driver

`lake exe templateTest` is the runtime half of the test suite (the compile-time
half — axiom audit, coverage gate — runs while the libraries build). Stages:

1. **Manifest**: dump every hand-written declaration under `Template` —
...
3. **Import roots**: every module under `Template/` must be imported by both
   `Template.lean` and `Template/AxiomAudit.lean` (the audit sweep only sees
...
`terminal` marks membership in `TemplateTest.terminalLedger` — the in-code
...
universe (`templateOmittedTokens` is consumed by the `#omitted_audit`
...
/-- Declarations under the root by `TemplateTest.auditRule` (the shared
copy of `inAuditedNamespace`, in `TemplateTest/Coverage.lean`), printed
...
`computeCoverage` applies `TemplateTest.auditRule` (the axiom sweep's rule,
...
def manifestEntries (root : Name) (cov : CoverageReport) : CoreM (Array Entry) := do
  ...
  for r in cov.pinRoots do
    for m in TemplateTest.cone env root cov.univ cov.depsOf [r] do
  ...
  for n in names do
    ...
    let terminal := TemplateTest.terminalLedger.any (·.1 == n)
...
/-- Modules under `Template/`, from the filesystem. -/
def sourceModules : IO (Array Name) := do
  let files ← FilePath.walkDir "Template"
...
are allowed, and an edge touching a module outside `Template/`'s
...
unsafe def main (args : List String) : IO UInt32 := do
  ...
  unless (← FilePath.pathExists "Template.lean") do
    throw <| IO.userError "run from the repo root (Template.lean not found)"
  ...
  -- `importAll`, matching `Template/AxiomAudit.lean`: a plain import resolves
  ...
  let env ← importModules
    #[{module := `Template, importAll := true},
      {module := `Template.AxiomAudit, importAll := true}]
    {} (trustLevel := 1024) (loadExts := true) (level := .private)
  let count := auditCount env `Template
  IO.println s!"templateTest: environment loaded, audit-rule count {count}"
  ...
  let cov := computeCoverage env `Template
  let ctx : Core.Context := { fileName := "<templateTest>", fileMap := default }
  let entries ← Prod.fst <$> (manifestEntries `Template cov).toIO ctx { env }
  ...
  IO.println s!"manifest-tally: {cov.univ.size} declarations under `Template` — \
  ...
  if update then
    IO.FS.writeFile lockFile
      ("-- Statement lock: elaborated types (and definition value hashes) of\n\
        -- every hand-written declaration under `Template`, printed by\n\
        -- `lake exe templateTest --update-lock`. Proof bodies of theorems are\n\
  ...
  -- for `Template.Accounting.useful`); a name bound in the row's own
  ...
  for (n, _) in env.constants.toList do
    ...
    if auditRule `Template n then
      ...
      for i in [0:comps.length] do
        ...
        templateSuffixesM := templateSuffixesM.insert suffix
  ...
  let templateSuffixes := templateSuffixesM
    ...
    knownRefs.contains t || allowedVocab.contains t
      ...
      || (match t.splitOn "." with
          | head :: rest@(_ :: _) =>
            ...
            (templateSuffixes.contains head && knownRefs.contains tail)
  ...
  for e in entries do
    ...
    if e.name == "Template.templateOmittedTokens" then continue
  ...
  unless !resolvesRef "T" "clearly_no_such_declaration_x9" &&
      resolvesRef "T" "Template.hello_world" do
  ...
  -- `TemplateTest/Gate.lean` joined when the coverage report moved there —
  -- its consumption graph only sees modules it `import all`s.
  let mods ← sourceModules
  let rootImports ← importsOf "Template.lean"
  let auditImports ← importsOf ("Template" / "AxiomAudit.lean")
  let gateImports ← importsOf ("TemplateTest" / "Gate.lean")
  for m in mods do
    unless rootImports.contains m do
      failures := failures.push ⟨"import-roots", s!"{m} not imported by Template.lean"⟩
    unless m == `Template.AxiomAudit || auditImports.contains m do
      failures := failures.push ⟨"import-roots", s!"{m} not imported by Template/AxiomAudit.lean"⟩
    unless gateImports.contains m do
      failures := failures.push ⟨"import-roots", s!"{m} not imported by TemplateTest/Gate.lean"⟩
  ...
  -- carrier directly, possibly through other Template modules — the
  ...
  if failures.isEmpty then
    IO.println "templateTest: all runtime stages green"
    return 0
  else
