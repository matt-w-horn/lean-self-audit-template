# The Template verification ritual: build, lint, lint-style, test, stamp.
...
# (.pre-commit-config.yaml, the workflows, Template/Lint.lean, and this
...
silencing-guard:
	...
	gatehits=$$(git diff $(GUARD_DIFF) -U0 -- .pre-commit-config.yaml '.github/workflows/*' Template/Lint.lean Makefile \
	  | grep -E '^[-+]' | grep -vE '^(\+\+\+|---)'; true); \
...
# replays each Template module's .olean through the kernel, imports
...
# the toolchain source), so `leanchecker Template` already checks all of
