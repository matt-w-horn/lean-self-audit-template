...
* `#axiom_budget_all Template` sweeps the declarations of the environment
  whose name root is `Template` (internal `_`-prefixed auxiliaries skipped,
  compiler-generated equation/match lemmas included).
* `#omitted_audit Template` is the negative-space complement: the results the
...
* The environment holds only what this file *imports*, and the import list
  ...
  `TemplateTest/Main.lean` does, by requiring every module under
  `Template/` to appear in both this file and `Template.lean`. The imports
...
`TemplateTest/Coverage.lean`'s `auditRule` carries a copy of these two
...
in the namespace of anyone who writes `import Template`. Either way it sits
outside the sweeps it powers — its name root is not `Template` — so the
...
analysis the library scopes out rather than axiomatizes. The template ships
one placeholder token, `exitTime`, which `tests/negative/OmittedTokenFixture.lean`
exercises; replace the list with your library's omitted families. Matching is
case-sensitive substring over fully-qualified names. -/
...
open Lean Elab Command in
/-- `#axiom_budget_all Template` runs the axiom-budget check on every
...
open Lean Elab Command in
/-- `#omitted_audit Template` fails elaboration if any declaration under the
given namespace root carries one of `Template.templateOmittedTokens` in its
name.
...
elab "#omitted_audit " pfx:ident : command => do
  ...
  for (n, _) in env.constants.toList do
    if inAuditedNamespace root n then
      ...
      for tok in Template.templateOmittedTokens do
